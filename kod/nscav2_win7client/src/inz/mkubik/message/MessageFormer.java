...
public class MessageFormer {
	...
	public byte[] formEstablishEncryption() {
		...
		messageCode[0] = (byte) StateUtils.Message.ESTABLISH_ENCRYPTION
				.ordinal();
		...
		byte[] initializationVector = CryptoManager.INSTANCE
				.getInitializationVector();
		byte[] totalData = StateUtils.combineByteArrays(messageCode,
				sizeOfSymmetricKeyAsByteArray);
		...
		byte[] sizeOfHashMessage = StateUtils
				.getSizeAsByteArray(hashOfMessage.length);
		totalData = StateUtils.combineByteArrays(sizeOfHashMessage, totalData);
		totalData = CryptoManager.INSTANCE
				.getRSAPublicKeyEncryptedLine(totalData);
	...
	public byte[] formClientID() {
		...
		System.arraycopy(clientIdWithAdd, 0, dataToBeEncrypted, 1,
				clientIdWithAdd.length);
		byte[] dataToSend = CryptoManager.INSTANCE
				.getRSAPublicKeyEncryptedLine(dataToBeEncrypted);
		...
		System.arraycopy(sizeAsByteArray, 0, totalData, 0,
				sizeAsByteArray.length);
		System.arraycopy(dataToSend, 0, totalData, sizeAsByteArray.length,
				dataToSend.length);
	...
	public byte[] formChosenAlgorithm() {
		byte[] dataToBeEncrypted = new byte[SharedPreferencesProxy
				.getInstance().getClientID().getBytes().length + 1];
		// byte[] dataToBeEncrypted = new
		// byte[StateUtils.getClientId().getBytes().length + 1];
		dataToBeEncrypted[0] = (byte) StateUtils.Message.CHOSEN_ALGORITHM
				.ordinal();
		System.arraycopy(StateUtils.getChosenAlgorithm().getBytes(), 0,
				dataToBeEncrypted, 1, StateUtils.getChosenAlgorithm()
						.getBytes().length);
		byte[] dataToSend = CryptoManager.INSTANCE
				.getRSAPublicKeyEncryptedLine(dataToBeEncrypted);
		byte[] sizeAsByteArray = new byte[StateUtils.constBytesSize];
	...
	public byte[] formProtocolName() {
		...
		dataToSend = protocol;
		byte[] sizeAsByteArray = StateUtils.getSizeAsByteArray(dataToSend
				.length() + 1);

		byte[] totalData = new byte[sizeAsByteArray.length
				+ dataToSend.getBytes().length + 1];
		System.arraycopy(sizeAsByteArray, 0, totalData, 0,
				sizeAsByteArray.length);
		totalData[StateUtils.constBytesSize] = (byte) StateUtils.Message.REQUEST_PROTOCOL
				.ordinal();
		System.arraycopy(dataToSend.getBytes(), 0, totalData,
				sizeAsByteArray.length + 1, dataToSend.getBytes().length);
		return totalData;
	}
	...
	public byte[] formLogin() {
		try {
			...
			code[1] = 1;
			byte[] login = SharedPreferencesProxy.getInstance().getLogin()
					.getBytes("UTF-8");
			byte[] totalData = StateUtils.combineByteArrays(code, login);
			byte[] hash = CryptoManager.INSTANCE.digest(totalData);
			byte[] sizeOfMessageHash = StateUtils
					.getSizeAsByteArray(hash.length);
			totalData = StateUtils.combineByteArrays(totalData, hash);
			totalData = StateUtils.combineByteArrays(sizeOfMessageHash,
					totalData);
			totalData = CryptoManager.INSTANCE
					.getAESKeyEncryptedLine(totalData);
			byte[] sizeOfMessage = StateUtils
					.getSizeAsByteArray(totalData.length);
			totalData = StateUtils.combineByteArrays(sizeOfMessage, totalData);
			return totalData;
		} catch (Exception e) {
			return null;
	...
	public byte[] formPassword() {
		try {
			...
			code[0] = (byte) StateUtils.Message.AUTH_DATA.ordinal();
			code[1] = 3;
			byte[] password = SharedPreferencesProxy.getInstance()
	...
	public byte[] formLog(DatabaseProvider databaseProvider) {
		try {
			...
			byte[] hostname = SharedPreferencesProxy.getInstance()
					.getHostName().getBytes("UTF-8");
			for (IcingaLog i : logs) {
				String value = i.getValue();
				String timestamp = i.getTimestamp().toString();
				...
				if (tablename.equals("BATTERY")) {
					...
					byte[] timestampAsByteArray = StateUtils
							.getTimeStampAsByteArray(integer);
					totalData = StateUtils.combineByteArrays(totalData,
					...
					byte[] output = StateUtils
							.getStateOfHostAsString(hostState);
					totalData = StateUtils.combineByteArrays(totalData, output);
					totalData = StateUtils.combineByteArrays(totalData, pipe);
					totalData = StateUtils
							.combineByteArrays(totalData, StateUtils
									.getStateOfHostAsStringWithState(hostState));
					totalData = StateUtils.combineByteArrays(totalData, zero);
				}
				...
				totalData = StateUtils
						.combineByteArrays(totalData, serviceName);
				totalData = StateUtils.combineByteArrays(totalData, zero);
				byte[] state = new byte[1];
				state[0] = (byte) Integer.parseInt(icingaLevel);
				totalData = StateUtils.combineByteArrays(totalData, state);
				byte[] output = value.getBytes("UTF-8");
				byte[] textToOutput = StateUtils.combineByteArrays(serviceName,
						"=".getBytes("UTF-8"));
				output = StateUtils.combineByteArrays(textToOutput, output);
				totalData = StateUtils.combineByteArrays(totalData, output);
				totalData = StateUtils.combineByteArrays(totalData, pipe);
				totalData = StateUtils.combineByteArrays(totalData,
						StateUtils.getTextForGraph(tablename));
				totalData = StateUtils.combineByteArrays(totalData,
						value.getBytes("UTF-8"));
				totalData = StateUtils.combineByteArrays(totalData,
						StateUtils.getTextAfterGraph(tablename));
				totalData = StateUtils.combineByteArrays(totalData, zero);
			}
