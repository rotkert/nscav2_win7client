...
public class SHAModule {
	...
	public SHAModule() {
		try {
			...
			messageDigest = MessageDigest
					.getInstance(SHAModule.DIGEST_ALGORITHM);
	 ...
	 * @param message text to be digested
	 ...
	 * @param hashedMessage hash of message (received from remote server)
	 * @param originalMessage message, which check we would like to check
	 ...
	 * @param hashedMessage hash of message (received from remote server)
	 * @param message message, which check we would like to check
	 * @return are these two the same?
	 */
	public boolean checkHashByPublicKey(byte[] hashedMessage, byte[] message,
			PublicKey publicKey) {
		try {
